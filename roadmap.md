...
| **Phase 3 — Strategy Analyst** | brandclaw-data-collection ✅, brandclaw-tool-orchestrator ✅, strategy-analyst-stub Phase A+B ✅ (gemerged 2026-05-18 via `a0e59a5b`); Phase C (Cron + cost-budget) **gesuperseded door het Agents-initiatief** → `agents-scheduling` (F2) + `agents-brandclaw-convergentie` (F3) | 15-20d landed | ✅ Phase A+B done; Phase C ⛔ gesuperseded 2026-07-06 |
...
| Fase | Task | Wanneer | Effort | Status |
|---|---|---|---|---|
| 1 | [`agents-foundation`](tasks/agents-foundation.md) — pluggable output-contract op `runAgentLoop` + `AgentRun`/`AgentArtifact` + code-registry + run-API | pre-launch NOW | 4-6d | open — **startpunt** (A1-spike eerst) |
| 1 | [`agents-motor-wiring`](tasks/agents-motor-wiring.md) — 5 persona-agents op bestaande motoren + Claw-tool-bridge + propose-only confirm + F-VAL-poort | pre-launch NOW | 4-6d | open (dep: foundation) |
| 1 | [`agents-ui-inbox`](tasks/agents-ui-inbox.md) — sidebar-sectie + catalogus + agent-detail (use-cases + Claw-panel-chat) + results-inbox | pre-launch NOW | 5-7d | open (dep: foundation; ≥1 agent voor e2e-smoke) |
| 1 | [`agents-data-analyst`](tasks/agents-data-analyst.md) — curated read-only query-tools + TABLE-artefact + tabel-render | pre-launch NOW | 3-5d | open (dep: foundation + ui-inbox) — **eerste drop-kandidaat** |
...
| ~~`strategy-analyst-stub` Phase C~~ | ~~Vercel Cron + concurrency-cap + cost-budget alerts~~ → **gesuperseded door het Agents-initiatief**: scheduling landt generiek als [`agents-scheduling`](tasks/agents-scheduling.md) (Fase 2, `AGENT_TASK`-brug), Strategy-Analyst-als-agent + per-workspace cost-budget als [`agents-brandclaw-convergentie`](tasks/agents-brandclaw-convergentie.md) (Fase 3). | — | — | ⛔ gesuperseded 2026-07-06 (zie §🤖 Agents) |
...
| [`stripe-billing-live`](tasks/stripe-billing-live.md) | Stripe live billing — checkout + webhooks + plan enforcement | 1w | #6 | ✅ **code-live-ready** (PR #79, 2026-07-06) — hardening done; wacht op Stripe-dashboard-config ([`docs/playbooks/stripe-go-live.md`](docs/playbooks/stripe-go-live.md)) |
