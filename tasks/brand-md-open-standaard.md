...
5. **Landingspagina + use-hub** (EN + NL): uitleg, generator-CTA, full-profile-documentatie, ruimhartige links naar de upstream-spec, én de **use-hub** (`/brandmd/use`): per tool (Claude/ChatGPT/Cursor/agents/any chat) een 30-sec-recept + test-prompt, publiek zonder e-mail. Zelfde content als het use-paneel op de resultaatpagina (touchpoint 1.6); de derde regel in elk gegenereerd bestand linkt ernaar.
...
- [ ] Use-hub live (`/brandmd/use`, per-tool-recepten, geen e-mail vereist) + use-paneel op de resultaatpagina direct na download; derde bestand-regel linkt naar de hub
