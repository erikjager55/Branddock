...
owner: claude-code
status: done
sub-cluster-done: A foundation + A engine + B v1 (POST /api/alignment/review-external + ingest helpers met SSRF-hardening + byte-cap streaming + html-strip 2-pass). Live smoke groen.
sub-cluster-todo: B-2 — file-upload (PDF via unpdf + DOCX via mammoth); C — Surface 1 Brand Alignment Tab 3 UI; D — Surface 2 Brand Assistant chat-tool (review_content read-tool); E — Surface 3 PublishGate uitbreiding (bevindingen-tabel render)
