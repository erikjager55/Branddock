...
2. ⚠️ **HERZIEN 2026-07-06 (user-verzoek, branch feat/agents-research-parity)**: deep research draait weer op de vólledige Library-default-config (6 queries/12 bronnen/verify aan/480s) — identiek aan de Knowledge-Library-ingang; de harde kill (570s) + run-guard 740s vangen het wallclock-risico. Oorspronkelijke afwijking: **deep research draaide op compacte config** (3 queries / 6 bronnen / verify uit / 5-min-deadline + harde kill op 6,5 min): de interne deep-research-deadline wordt niet door elke fase gerespecteerd (8-min-config liep 15+ min). Volle-diepte research blijft de Knowledge-Library-ingang.
