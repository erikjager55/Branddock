...
> **Status: ✅ LIVE OP PRODUCTIE — go-live 2026-07-06 (changelog #357 hardening + #366 go-live).** Deze task-file (2026-05-07) beschreef een from-scratch-bouw, maar een audit toonde dat de subscription-lifecycle al code-compleet + gewired was (checkout → webhook HMAC+idempotency → DB-sync → planTier → enforcement, portal, invoice-sync, live BillingTab). De hardening-werkstroom dichtte de resterende bugs/gaten: **S1** gratis-upgrade-exploit verwijderd + one-time-purchase-completion gewired (`payment_intent.succeeded`); **S2** factuur-/100 + yearly-mis-charge; **S3** echte usage-meter + env fail-fast; **S4** [go-live-playbook](../docs/playbooks/stripe-go-live.md). Commits 255b39de/c90ed171/ca0517d4 (+ deze docs-commit), branch `feat/stripe-billing-hardening`. **Go-live gedaan** (2026-07-06): betterbrands.nl Stripe-account, 3 live-producten/prijzen (PRO €29/AGENCY €99/ENTERPRISE €249), live-keys, webhook (9 events, enabled) + Customer Portal + `NEXT_PUBLIC_BILLING_ENABLED=true` in Vercel — billing draait **LIVE** op branddock-7y9n.vercel.app (checkout→PRO / cancel→FREE end-to-end getest). Plus 2 checkout-redirect-404-fixes (#85/#86) + billing-styling (#88). **Uit scope (per-token-fase)**: metered-overage/usage-metering/trial/PaymentMethod-sync. De acceptatiecriteria hieronder zijn grotendeels via de bestaande code al vervuld; de `WorkspacePlan`-enum heet in de praktijk `PlanTier` (FREE/PRO/AGENCY/ENTERPRISE).
