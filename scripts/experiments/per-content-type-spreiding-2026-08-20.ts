...
// Aantal generaties per (content-type, model). De meting van 20-08 draaide er
// ÉÉN, en dat bleek te weinig: twee ongewijzigde controle-modellen dreven
// gemiddeld 4,0 punten (uitschieter 13) terwijl de winnaars 1-4 punten uit
// elkaar liggen. Met één sample kun je die twee niet uit elkaar houden.
const SAMPLES = Number(process.env.SAMPLES ?? 5);

const MODELS: ModelConfig[] = [
  // Herijking 2026-08-20. Drie modellen zijn sinds de meting van 13-05 gewisseld
  // (#226, modellen-refresh 21-07); twee zijn ONGEWIJZIGD en dienen daarom als
  // CONTROLE: halen Haiku en Gemini Pro dezelfde score als in mei, dan is de
  // methode stabiel en zijn de verschillen bij de gewisselde modellen echt.
  { id: 'opus-4-8', label: 'Claude Opus 4.8 + thinking', provider: 'anthropic', model: 'claude-opus-4-8', withThinking: true },
  { id: 'sonnet-5', label: 'Claude Sonnet 5 + thinking', provider: 'anthropic', model: 'claude-sonnet-5', withThinking: true },
  { id: 'haiku-4-5', label: 'Claude Haiku 4.5 (CONTROLE — ongewijzigd)', provider: 'anthropic', model: 'claude-haiku-4-5-20251001' },
  { id: 'gpt-5-6', label: 'GPT-5.6', provider: 'openai', model: 'gpt-5.6' },
  { id: 'gemini-3-1-pro', label: 'Gemini 3.1 Pro + thinking (CONTROLE — ongewijzigd)', provider: 'google', model: 'gemini-3.1-pro-preview', withThinking: true },
  // Vervangt GPT-5.4 Mini uit de mei-meting: er is geen gpt-5.6-mini in de
  // codebase. gemini-3.5-flash is de goedkope laag die de app wél gebruikt.
  { id: 'gemini-3-5-flash', label: 'Gemini 3.5 Flash (vervangt GPT-5.4 Mini)', provider: 'google', model: 'gemini-3.5-flash' },
];

// ─── Cost-estimate per provider/model (USD per 1M tokens) ──
// ⚠️ PRIJZEN NIET GEVERIFIEERD voor de juli-generatie. Overgenomen van het
// gelijknamige model uit de mei-meting waar dat kon; de kostenkolom in het
// rapport is dus INDICATIEF en mag niet als inkoopcijfer gelezen worden. De
// waarde van deze herijking zit in de kwaliteitsscores, niet in de kosten.
...
async function callAnthropic(
  ...
  // Herijking 20-08: `temperature` is DEPRECATED voor claude-sonnet-5 — de oude
  // tak gaf daar een 400 ("`temperature` is deprecated for this model"). Sonnet-5
  // hoort dus in de moderne tak, die bovendien de tegenhanger is van de
  // "Sonnet 4.6 + thinking"-conditie uit de mei-meting.
  // ⚠️ Methodologische noot voor het rapport: de mei-run stuurde temperature 0.7
  // mee, deze niet. Dat is geen keuze maar een API-wijziging, en het is een
  // caveat op de vergelijkbaarheid.
  const isModernThinking = /opus-4-7|opus-4-8|opus-5|sonnet-5/.test(model);
  let params: unknown;
  if (withThinking && isModernThinking) {
    params = {
      model,
...
Wees streng. Range 30-95 gebruiken.`;
  ...
  // ⚠️ De JUDGE blijft bewust claude-sonnet-4-6 — hetzelfde model als de
  // mei-meting. Alleen dan is het verschil in scores toe te schrijven aan de
  // GENERATIE-modellen en niet aan een verschoven meetlat.
  //
  // Dit stond even op sonnet-5 door een globale replace bij het herijken van de
  // MODELS-array. Dat zou de hele vergelijking met mei stilzwijgend ongeldig
  // hebben gemaakt: andere judge, andere schaal, zelfde ogende getallen.
...
async function main() {
  ...
  // LIMIT_TYPES beperkt de run voor een goedkope proefdraai; leeg = alle acht.
  const TE_DRAAIEN = CONTENT_TYPES.slice(0, Number(process.env.LIMIT_TYPES ?? CONTENT_TYPES.length));
  for (const brief of TE_DRAAIEN) {
    console.log(`\n--- Content-type: ${brief.id} (${brief.category}) ---`);
   for (let sample = 1; sample <= SAMPLES; sample++) {
    console.log(`  [sample ${sample}/${SAMPLES}]`);
    ...
    (judgesByContentType[brief.id] ??= []).push(...scores.map((x) => ({ ...x, sample })));
    for (const s of scores) {
      console.log(`    ${s.modelId}: composite ${s.composite}`);
    }
   }
  }
  ...
  writeFileSync(
    resolve(outDir, '2026-08-20-spreiding-raw.json'),
    JSON.stringify({ samples: SAMPLES, results: allResults, judges: judgesByContentType }, null, 2),
  );

  // ── Spreidingsrapport ──────────────────────────────────────────────────────
  // Eén getal per model verbergt precies wat we willen weten. Per (content-type,
  // model) staan hier het gemiddelde, de min-max en de standaarddeviatie over
  // SAMPLES generaties.
  const agg = (xs: number[]) => {
  ...
  const L: string[] = [];
  L.push(`# Model-routing — spreidingsmeting 2026-08-20`);
  L.push('');
  L.push(`Vervolg op de herijking van dezelfde dag. Daar draaide **één** generatie per`);
  L.push(`(content-type, model); hier **${SAMPLES}**. Zelfde judge (claude-sonnet-4-6),`);
  L.push(`zelfde prompts.`);
  L.push('');
  L.push(`**De vraag**: de herijking liet 4,0 punten drift zien op ONGEWIJZIGDE modellen,`);
  L.push(`terwijl de winnaars 1-4 punten uit elkaar lagen. Kan deze meting dat verschil`);
  L.push(`überhaupt aantonen? Een standaarddeviatie per conditie beantwoordt dat.`);
  L.push('');
  ...
  for (const brief of TE_DRAAIEN) {
    ...
    L.push(`### ${brief.id} — ${brief.category}`);
    L.push('');
    L.push('| Model | gem. | min–max | sd | n |');
    L.push('|---|---:|:---:|---:|---:|');
    const perModel = new Map<string, number[]>();
    for (const s of scores) {
      if (!perModel.has(s.modelId)) perModel.set(s.modelId, []);
      perModel.get(s.modelId)!.push(s.composite);
    }
    const rijen = [...perModel.entries()].map(([id, xs]) => ({ id, ...agg(xs) }))
      .sort((a, b) => b.mean - a.mean);
    for (const r of rijen) {
      alleSd.push(r.sd);
      const label = MODELS.find((m) => m.id === r.id)?.label ?? r.id;
      L.push(`| ${label} | **${r.mean.toFixed(1)}** | ${r.min}–${r.max} | ${r.sd.toFixed(1)} | ${r.n} |`);
    }
    const top = rijen[0], tweede = rijen[1];
    if (top && tweede) {
      const verschil = top.mean - tweede.mean;
      const pooled = Math.sqrt((top.sd ** 2 + tweede.sd ** 2) / 2);
      L.push('');
      L.push(`Verschil tussen 1 en 2: **${verschil.toFixed(1)}** punt, gepoolde sd **${pooled.toFixed(1)}**. ` +
        (verschil > 2 * pooled
          ? `Groter dan 2 sd — dit verschil is aantoonbaar.`
          : `**Binnen 2 sd — dit verschil is met ${SAMPLES} samples niet aantoonbaar.**`));
    }
    L.push('');
  }

  const gemSd = alleSd.reduce((a, b) => a + b, 0) / Math.max(1, alleSd.length);
  L.push('## Conclusie');
  L.push('');
  L.push(`Gemiddelde standaarddeviatie over alle ${alleSd.length} condities: **${gemSd.toFixed(1)} punten**.`);
  L.push('');
  L.push(`Een verschil is pas aantoonbaar bij ruwweg 2 sd, dus **${(2 * gemSd).toFixed(1)} punten**.`);
  L.push(`De winnaars in de routingtabel liggen 1-4 punten uit elkaar.`);
  L.push('');
  writeFileSync(resolve(outDir, '2026-08-20-spreiding-report.md'), L.join('\n'));
  console.log(`\nReport: docs/experiments/2026-08-20-spreiding-report.md`);
  console.log(`Gemiddelde sd: ${gemSd.toFixed(1)} punten over ${alleSd.length} condities`);
}
