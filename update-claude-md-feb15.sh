#!/bin/bash
# Update CLAUDE.md — 15 februari 2026 (avond)
set -e
...
│   ├── EnhancedSidebarSimple.tsx        ← Sidebar (refactored: useBrandAssets)
...
13. **`mockBrandAssets` → `useBrandAssets()` in 12 componenten** (EnhancedSidebarSimple, ResearchHubEnhanced, StrategicResearchPlanner, UniversalAssetDashboard, ResearchTargetSelector, ResearchDashboard, AssetUnlockDetailView, CampaignStrategyGeneratorDetail, UniversalStrategyGenerator, StrategicReport, QuestionnaireWorkflowStep, InterviewWorkflowStep)
...
- **Pre-existing type issues**: ResearchItem/Persona mismatches in ResearchHubEnhanced, PersonasSection, SocialRelevancyDashboard
...
**mockBrandAssets (16 imports totaal):**
- ✅ 12 componenten → `useBrandAssets()` context hook
- ⬜ 4 utils/services → parametrisch maken (brandAssets als parameter):
  - `src/utils/campaign-decision-gate.ts`
  - `src/utils/campaign-decision-calculator.ts` + `v2`
  - `src/utils/platform-decision-aggregator.ts`
- ⬜ 2 services → parametrisch maken:
  - `src/services/GlobalSearchService.ts`
  - `src/services/SmartSuggestionsService.ts`
  - `src/services/RelationshipService.ts`
- ⬜ 1 hook → `useBrandAssets()` intern:
  - `src/hooks/useBreadcrumbs.ts`
- ⬜ 1 example → low priority:
  - `src/examples/decision-layer-integration-example.tsx`
- ⬜ 1 context (eigen mock fallback — OK):
  - `src/contexts/BrandAssetsContext.tsx`

**mockPersonas (12 imports totaal):**
- ⬜ Componenten → `usePersonas()` context hook
- ⬜ Utils/services → parametrisch maken

**Overige mocks (nog niet gestart):**
- `research-bundles` (7 imports)
- `mock-trends` (4 imports)
- `mock-knowledge` / `knowledge-resources` (4 imports)
- `mock-products` (2 imports)
- `mock-campaigns` (2 imports)
- `strategy-tools` (2 imports)
- `mock-collaboration` (1 import)
- `mock-activities` (1 import)
- `mock-bundles` (1 import)
- `mock-decision-analysis` (1 import)
- `mock-relationships` (1 import)
...
echo "✅ CLAUDE.md bijgewerkt (15 feb 2026 — avond)"
echo ""
echo "Commit:"
echo "  git add CLAUDE.md && git commit -m 'docs: update CLAUDE.md — mockBrandAssets refactor done, roadmap bijgewerkt'"
