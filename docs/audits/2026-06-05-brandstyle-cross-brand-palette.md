...
## 10. Strenge framework-match tegen "geleende" usage (2026-06-05, commit volgt, #285)
Na de #283-fix + clean dev-server-restart toonde een verse Napking re-scrape (mét multi-page usage-data, `multi-page=yes`) dat WP-admin #007CBA viel maar **#ABB8C3 bleef**. Diagnose (live curl napking.nl + afstandsberekening): #ABB8C3 (Gutenberg-default, rendert zélf nergens) ligt ~33 van de echt-gerenderde Tailwind-grijs #9CA3AF en ~31 van #A2B8A5 — binnen de losse `MATCH_TOLERANCE` (40) — en absorbeerde zo het gebruik van z'n buren → false-`strong`.

**Fix** (`palette-usage-filter.ts`): `renderStrength` kreeg een `tolerance`-param; `STRICT_FRAMEWORK_TOLERANCE = 6`. `keep()`: mét multi-page-data behoudt een framework-default alléén via een near-exact eigen render (strict 6); zónder multi-page valt het terug op #283-pixel-pass-gedrag. Computed color/bg/border zijn exacte RGB (geen anti-aliasing), dus een ECHT toegepaste framework-kleur rendert op dist 0 en blijft. **Adversariële review**: ronde-1 ving severe over-drop (strict-only negeerde pixel-pass in default-config zonder screenshotter) + onder-drop (#ABB8C3↔#ADB5BD dist 7) → pixel-pad hersteld voor no-multi-page + tolerantie 12→6; ronde-2 SHIP. Smoke `phase51` 21/21. Napking-exact: #ABB8C3 valt in multi-page én pixel-only.

**Belangrijke meta-les (stale dev-server)**: de re-scrape vóór deze diagnose draaide op stale code (dev-server 4u oud, hield de dynamisch-geïmporteerde `palette-usage-filter` vast terwijl analysis-engine.ts ververste). **Patroon: brandstyle-fix smoke-groen maar re-scrape onveranderd → verdenk stale dev-server, clean restart vóór je concludeert.**

**Volgende: re-scrape Napking → palet schoon (Ocean Blue + charcoal/soft-white/slate/brown, geen #ABB8C3).**
