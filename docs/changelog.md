...
### 407. Marketing: placeholder-quote verborgen + domein-koppel-runbook (branddock.app)

Twee taak-#9-punten. **(1) Pilot-quote verborgen**: de homepage toonde een verzonnen quote met "Pilot customer placeholder · CMO at a B2B SaaS scale-up" — misleidende social proof, mag niet live. `<CustomerQuote />` niet meer gerenderd (component + een naam/rol/bedrijf-template blijven klaarstaan; terugzetten zodra er een échte quote is). **(2) Domein-runbook**: `docs/playbooks/custom-domain-branddock-app.md` — Erik registreerde `branddock.app` en wil de app op een subdomein. Runbook legt de beoogde structuur vast (app op `app.branddock.app`, marketing op de apex, landingspagina's op `<workspace>.branddock.app`), Eriks infra-stappen (Vercel-domeinen, DNS-records, env-vars `BETTER_AUTH_URL`/`NEXT_PUBLIC_MARKETING_URL`/`NEXT_PUBLIC_APP_URL`, Meta-OAuth-redirect-URI) én de code-aanpassingen die deze keuze vergt (host-router: `app.` als app-host + apex→marketing-rewrite, reserved-slug-guard voor `app`, absolute marketing-CTA's) — bewust als aparte PR op Eriks go, want pas verifieerbaar tegen het echte domein. Wijkt af van ADR 2026-05-22 (apex=app-shell) — expliciet genoteerd.
