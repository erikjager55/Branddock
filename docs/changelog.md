...
### 408. Domein-launch-prep branddock.app — routing + reserved-slugs + placeholder-quote verborgen

Voorbereiding voor het koppelen van `branddock.app` (Erik registreerde het domein; keuze: app op subdomein, marketing op de apex). Alle wijzigingen zijn **inert tot de domeinen live zijn** — de huidige `branddock-7y9n.vercel.app` verandert niet (valt door alle host-checks → passthrough, geverifieerd). **host-router**: `app.branddock.app` → app-shell (niet als workspace-slug `app` geparsed), apex/www root `/` → `/marketing`, `<ws>.branddock.app` → landingspagina's (ongewijzigd). **Reserved-slug-guard** in workspace-create: `app`/`www`/`api`/`admin`/`p`/`static`/`assets` → 409 (voorkomt botsing met app/infra-hosts). **Marketing-CTA's** (5×) via nieuwe `app-url.ts`-helper absoluut naar `NEXT_PUBLIC_APP_URL` zodra gezet, anders relatief (blijft nu werken). **Placeholder-quote verborgen**: de verzonnen homepage-quote ("Pilot customer placeholder · CMO…") is misleidende social proof — `<CustomerQuote />` niet meer gerenderd (component + template blijven klaar). **Runbook** `docs/playbooks/custom-domain-branddock-app.md` met Eriks resterende infra-stappen (Vercel-domeinen, DNS, env-vars `BETTER_AUTH_URL`/`NEXT_PUBLIC_MARKETING_URL`/`NEXT_PUBLIC_APP_URL`, Meta-OAuth-redirect). Wijkt bewust af van ADR 2026-05-22 (apex=app-shell) — genoteerd in de host-router-header. Bewijs: smoke `web-page-builder-phase4` 48/48 (incl. nieuwe apex-root→/marketing + app-host-passthrough-cases), tsc 0, eslint 0. Vervangt de gesloten PR #148 (changelog-conflict + gebundeld met de routing-code).
