...
## Aanvulling 2026-07-14 — D7 verfijnd: event-driven trede (Vera-triggers)

- **D7-autonomie-trap krijgt zijn derde trede expliciet**: on-demand → scheduled (Fase 2, live) → **event-driven propose-only** → proactief/Brandclaw (Fase 3). Event-driven runs (eerste toepassing: Vera-brand-review bij DAM-upload en deliverable→review) blijven volledig binnen de no-autonomy-regel: propose-only, geen writes zonder confirm, output uitsluitend als artefacten in de bestaande inbox. Een event-trigger is architectonisch een derde *dispatch-bron* voor dezelfde `runAgent()`-keten (`triggerType: 'event_driven'` — het contract anticipeerde hierop), géén nieuwe loop-variant.
- **Geverifieerd trigger-event (correctie op de idea-file)**: `PipelineStatus.REVIEW` is dood — nergens in levende code geschreven. Het levende "klaar voor review"-moment is de **`approvalStatus`-transitie naar `IN_REVIEW`** (`studio/[deliverableId]/approval/route.ts` + `campaigns/[id]/canvas/bulk-approve/route.ts`). Dáár komen de hooks; de dode enum-restanten opruimen is een aparte task.
- **Credits**: `AGENT_EVENT_REVIEW`-runs zijn **floor-gedekt 0 credits** — zelfde klasse als ALIGNMENT_SCAN en de andere analyse-agents (D8-lijn "merkcontext en validatie nooit meteren"). Herziening alleen via een nieuwe ADR-aanvulling.
- **Kosten-gate = opt-in**: beide triggers default **UIT**, per workspace per trigger (`AgentEventTriggerConfig`); de toggle-copy benoemt dat elke run AI-kosten maakt. Geen default-aan rollout, geen aan-bij-onboarding.
- **Moeheid-ontwerp als productinvariant** (drievoud): (1) 1 event-burst = 1 gebatchte run = max 1 melding (idempotencyKey-bucket + scheduledAt-delay op het bestaande `dispatchJob`-dedupe-gedrag); (2) notificatie alleen bij issues — een on-brand-run is een stil inbox-item; (3) harde daily cap per workspace met éénmalige cap-melding, geen stille drop.
- **Beslispunt cap-melding beslist**: één nieuw enum-lid **`NotificationType.AGENT_TRIGGER_CAP_REACHED`** (in dezelfde toch-al-vereiste db-push). `AGENT_RUN_COMPLETED` hergebruiken zou "Vera finished a task" liegen terwijl er juist géén run draaide, en de geen-stille-drop-eis is hard.
- **Gating ongewijzigd**: deze aanvulling legt de architectuur vast maar de bouw blijft gegate op de **Fase-0-concierge-validatie** (go/no-go-criteria in `tasks/agent-vera-triggers.md`). Eerste meting 2026-07-14: DAM-uploads op de pilot-workspace 9 stuks in één burst (5-7 juli), daarna stil; `IN_REVIEW`-transities 0 in 8 weken op heel prod — de review-trigger heeft vandaag geen voer. Observatiewindow loopt.
