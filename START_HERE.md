...
**Parallelle pre-launch-werkstroom (nieuw, 2026-06-30)** — Meertaligheid Fase 1-3 (`i18n-ui-foundation` → `content-locale-foundation` → `content-locale-target-picker`, + `i18n-ai-translation-pipeline`): startklaar en niet-brekend. User koos dit volledig op te pakken; het verandert het kritieke pad niet (Track C/`vercel-deployment` blijft de hard blocker). Begin met `i18n-ui-foundation` in plan-mode. Detail: roadmap §🌍 Meertaligheid + ADR `2026-06-28`.
