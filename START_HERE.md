...
1. **▶️ Agents — Fase 1 LIVE; dogfood-ronde 1 (2026-07-07) + ronde 2 (2026-07-12) ✅ GEDAAN** (changelog #359–#365 + #375; rapporten `docs/reports/agents-dogfood-2026-07-07.md` + `-2026-07-12.md`). Guardrails groen en herhaald: ~$0,09–0,10/run, eindcontent-F-VAL 73–76 (>70), en **credit-metering op agents praktijk-gevalideerd** (0-charge voor gratis agents/proposals; confirm-pad −3 credits, idempotent). **Ronde-2-hoofdvondst**: de ronde-1-strategist-fix (32k) was een fatale regressie (Anthropic SDK weigert non-streaming >21.333 tokens) — Stella was 5 dagen kapot op prod; **fix geleverd + gevalideerd** (loop-clamp + definitie 21.333, gotcha 2026-07-12), PR moet nog gemerged/gedeployed.
   - **Open uit de dogfood** (klein): (a) sweep op **productie** herhalen met `EXA`/`S2`-keys (user-held); (b) angle-generator-truncatie echt fixen (Gemini thinking-tokens in `maxOutputTokens`-budget); (c) strategist-subtool per-step maxTokens-override (gedeelde 16k-default niet globaal bumpen); (d) F-VAL-poort/score op rauwe REPORTs. Fase 2 `agents-scheduling` blijft gegate op pilot-adoptiedata; Fase 3 blocked.
