...
1. **Agents Fase 1 — start `agents-foundation`** (4-6d; Fase 1 totaal 4 tasks / 16-24d, lite-fallback ~11-17d) — nieuw initiatief 2026-07-05, user-prioriteit. Worktree `branddock-feat-agents-feature`; volgorde: `agents-foundation` (A1-spike eerst: pluggable output-contract zonder Strategy-Analyst-regressie) → `agents-motor-wiring` + `agents-ui-inbox` → `agents-data-analyst` (eerste drop-kandidaat). Start in plan-mode. Detail: roadmap §🤖 + ADR `2026-07-05-agents-architectuur`.
...
3. **~~Track B Phase C~~ → gesuperseded door het Agents-initiatief.** De Strategy-Analyst-cron/cost-budget wordt niet los gebouwd: scheduling landt generiek als `agents-scheduling` (Fase 2, ná de Agents-Fase-1-MVP), Strategy-Analyst-als-agent + cost-budget als `agents-brandclaw-convergentie` (Fase 3, blocked). Zie roadmap §🤖 Agents.
