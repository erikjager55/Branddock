...
1. **▶️ Agents — Fase 1 LIVE; dogfood-ronde 1 (2026-07-07) + ronde 2 (2026-07-12) ✅ GEDAAN** (changelog #359–#365 + #375–#376; rapporten `docs/reports/agents-dogfood-2026-07-07.md` + `-2026-07-12.md`). Guardrails groen en herhaald: ~$0,09–0,10/run, eindcontent-F-VAL 73–76 (>70), en **credit-metering op agents praktijk-gevalideerd** (0-charge voor gratis agents/proposals; confirm-pad −3 credits, idempotent). **Ronde-2-hoofdvondst**: de ronde-1-strategist-fix (32k) was een fatale regressie (Anthropic SDK weigert non-streaming >21.333 tokens) — Stella was 5 dagen kapot op prod; **fix gemerged + gedeployed** (PR #102, loop-clamp + definitie 21.333, gotcha 2026-07-12). **Follow-up-fixes ✅ (#376)**: angle-generator (Gemini-thinking uit) + strategist-foundation-budget 16k→24k, beide met echte runs gevalideerd; `worktree.sh --done` geïmplementeerd.
   - **Open uit de dogfood**: (a) sweep op **productie** herhalen met `EXA`/`S2`-keys (user-held); (b) F-VAL-poort/score op rauwe REPORTs (product-keuze). Fase 2 `agents-scheduling` blijft gegate op pilot-adoptiedata; Fase 3 blocked.
   ...
   - Detail: changelog #359–#365 + #375–#376, `tasks/done/agents-*.md` (incl. `agents-dogfood-ronde2.md`), memory `agents-initiative`.
