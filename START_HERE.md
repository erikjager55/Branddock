...
1. **Agents Fase 1 — `agents-foundation` ✅ DONE 2026-07-06** (#359, branch `feat/agents-foundation` — merge naar main is de volgende stap) — A1 bewezen (Strategy Analyst regressievrij), 5 review-rondes/10 reviewers 0 CRITICAL, alle 22 WARNINGs gefixt, gates + smoke groen. **Volgende**: branch mergen, dan `agents-motor-wiring` + `agents-ui-inbox` (parallel mogelijk) → `agents-data-analyst` (eerste drop-kandidaat). Detail: roadmap §🤖 + `tasks/done/agents-foundation.md`. ⚠️ Rollout: handmatige Neon `prisma db push` vóór deploy-verkeer (nieuwe modellen + `ResourceSource.AGENT`).
