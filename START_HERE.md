...
> **Stand 2026-07-05**: `vercel-deployment` ✅ gerealiseerd — de hard launch-blocker is weg. Het kritieke pad verschuift naar `stripe-billing-live` + `pilot-onboarding-better-brands`; de door de user gekozen actieve werkstroom is **Agents Fase 1** (parallel, eigen worktree).

1. **Agents Fase 1 ✅ COMPLEET 2026-07-06** (#359-#362, alles gemerged + gepusht naar production) — 6 persona-agents live (Nova/Vera/Stella/Milo/Marco/Dana): foundation, motor-wiring (propose-only confirm-keten), volledige Agents-UI + Claw-scoping, Data Analyst met server-owned TABLE-artefacten. Review-totaal: 10 reviewer-runs, 3 CRITICAL + 22 WARNINGs gefixt. **Volgende**: dogfooden met echte workflows; daarna `agents-scheduling` (Fase 2, gate: Fase 1-adoptie). Detail: changelog #359-#362 + tasks/done/agents-*.md.
