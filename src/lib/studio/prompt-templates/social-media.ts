...
- [ ] Would this poll generate 50+ comments explaining people's reasoning?`,
    ...
    buildUserPrompt: (params) =>
      buildSocialUserPrompt(
        ...
        'Platform: LinkedIn Poll. Output exactly these component groups: (1) "context" — 30-50 word framing paragraph above the poll, 2-3 sentences setting up the debate. (2) "question" — the poll question, plain text, **HARD LIMIT 140 characters** (LinkedIn rejects longer). No markdown, no quotes around it. Count characters before outputting; if longer, rewrite shorter. (3) "option-1" — first option string, **HARD LIMIT 30 characters** (LinkedIn truncates with "…" on mobile beyond this). Count characters; if longer, rewrite shorter — never pad with filler. (4) "option-2" — second option string, **HARD LIMIT 30 characters**. (5) "option-3" — OPTIONAL third option, max 30 chars; emit only when the question genuinely benefits from 3+ choices. (6) "option-4" — OPTIONAL fourth option (e.g. "It depends" / "None of the above"), max 30 chars. (7) "follow-up-comment" — 40-60 word first comment the author should post immediately after going live, explaining their own position and inviting others to share theirs. (8) "hashtags" — 2-3 topic-relevant hashtags. CRITICAL: NEVER bundle the question and options into a single body field; the preview renders each group separately. CRITICAL: every option-* MUST be ≤30 characters — measure the string length before outputting, rewrite if over.',
      ),
  },
