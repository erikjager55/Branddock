...
function buildBriefingSection(briefing?: CampaignBriefing): string {
  ...
  if (readySources.length > 0) {
    const sourceBlocks = readySources.map((source, idx) => {
      ...
      const origin = source.url ?? source.fileName ?? (source.type === 'text' ? 'manual entry' : 'attachment');
      ...
      const typeLabel = source.type === 'url' ? 'URL' : source.type === 'pdf' ? 'PDF' : 'Text';
...
// =============================================================================
// Strategy Foundation Prompt Builders
// Phase 1: Briefing Validation
// Phase 2: Strategy Foundation (enrichment + behavioral diagnosis)
// (Legacy Phase 4-6 hook prompts removed — see CQP pipeline below for current
//  insight mining + creative leap + creative debate builders.)
// =============================================================================
