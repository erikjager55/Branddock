...
export const writeTools: ClawToolDefinition[] = [
  ...
  // returned in the result so the AI corrects course.
  {
    ...
    description:
      'Fill or update type-specific Content Brief inputs on a deliverable. Keys depend on content type. **Examples per category** (NOT exhaustive — always check `availableContentTypeKeys` from inspect_current_entity for the actual valid keys for THIS deliverable): SEO keyword + secondary keywords + meta description (blog/web), hashtag strategy + emoji + post type (social), article structure + reading level (long-form), slide count + slide format (carousel), subject line + preview-text + CTA placement (email), value proposition + hook format + payoff promise + target objection + proof point (conversion content), **content-style fields including footage type + text overlay + color grade + caption style + hook strategy + narrative arc + video duration** (video / ad / paid-content), structure + quote count (PR), sales angle (sales), ad format (ad subtype). NEVER guess keys; always call `inspect_current_entity` first and use only keys from `availableContentTypeKeys` or already in `contentTypeInputs`. **Content-style fields (textOverlay, colorGrade, footageType, captionStyle, hookStrategy, etc.) BELONG HERE** — they are NOT visual-brief settings. The Visual Brief tool (`update_deliverable_visual_brief`) handles ONLY the Step 1 chip selection (lifestyle / quote-text / infographic / etc.). For tone / CTA wording use `update_deliverable_brief`. Boolean fields use true/false; arrays use string lists; everything else is a string.',
    inputSchema: z.object({
      deliverableId: z.string().describe('The deliverable ID from the Current Page context'),
      updates: z
