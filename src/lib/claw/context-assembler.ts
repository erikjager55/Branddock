...
function formatPageContext(ctx: ClawPageContext): string {
  ...
  if (ctx.entityType && ctx.entityId) {
    ...
    if (ctx.entityType === 'deliverable') {
      ...
      lines.push(
        '  2. Three write-tools cover Step 1, one per section: (a) `update_deliverable_brief` for the FOUR strategic textareas: objective, keyMessage, toneDirection, callToAction — always propose ALL FOUR in one call unless one is already non-empty. (b) `update_deliverable_content_inputs` for type-specific keys — ONLY use keys returned by inspect_current_entity, never invent keys. (c) `update_deliverable_visual_brief` for the Visual Brief subsection. All three apply via a user confirmation card — propose values grounded in brand context + persona psychographics, never generic filler.'
      );
      lines.push(
        '  3. CRITICAL when the user asks broadly ("vul de velden", "fill in everything", "complete the brief", "stel de briefing in", "geef suggesties"): propose updates across ALL THREE sections in a single turn — call all three write-tools in parallel. For `update_deliverable_brief` specifically: propose all 4 strategic fields (objective + keyMessage + toneDirection + callToAction) in that single call, never just 1 or 2. Skip a tool only when its section is already complete OR when no relevant context exists. Never stop after only one section unless the user was explicit ("only the visual brief", "alleen de tone").'
      );
      lines.push(
