...
function formatPageContext(ctx: ClawPageContext): string {
  ...
  if (ctx.entityType && ctx.entityId) {
    ...
    if (ctx.entityType === 'deliverable') {
      ...
      lines.push(
        '  3. CRITICAL when the user asks broadly ("vul de velden", "fill in everything", "complete the brief", "stel de briefing in", "geef suggesties"): propose updates across the TWO strategic sections (brief + contentTypeInputs) in a single turn — call both write-tools in parallel. For `update_deliverable_brief` specifically: propose all 4 strategic fields (objective + keyMessage + toneDirection + callToAction) in that single call, never just 1 or 2. Skip a tool only when its section is already complete OR when no relevant context exists. Do NOT call `update_deliverable_visual_brief` on these broad requests — the Visual Brief has its own user-driven entry point: the "Suggest setup from content" button in the Visual Brief panel. Mention that button in your text response so the user knows where to go.'
      );
      lines.push(
