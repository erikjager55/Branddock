  ...
  if (ctx.entityType && ctx.entityId) {
    ...
    if (ctx.entityType === 'deliverable') {
      ...
      if (isPuckRenderable(ctx.contentType, ctx.contentTypeInputs)) {
        ...
        lines.push(
          `  5. This is a **Puck-renderable page** (${ctx.contentType}) — its COPY and STRUCTURE are directly editable. (a) ALWAYS call \`read_landing_page_content\` first: it returns the section list (ids, types, order, locks) + the exact text-field paths. (b) Text changes ("maak de hero-kop korter", "punchier CTA"): propose via \`update_landing_page_content\` using ONLY returned paths. (c) Structure changes ("voeg een FAQ toe", "zet de testimonial hoger", "haal die sectie weg"): propose via \`update_landing_page_structure\` (add/remove/move/duplicate; guards: required sections per page type and locked sections are refused — explain refusals to the user). After adding a section, immediately fill its placeholder copy via a follow-up text edit. Still out of scope: images, links/hrefs, colors, layout-styling (advise the layout editor). Ground every change in the brand voice + tone.`
        );
      } else {
        lines.push(
