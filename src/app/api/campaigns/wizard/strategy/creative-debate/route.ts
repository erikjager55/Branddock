...
import { buildCreativePipelineContext, runCreativeDebate } from '@/lib/campaigns/strategy-chain';
...
/**
 * POST /api/campaigns/wizard/strategy/creative-debate
 * Runs a creative debate (critique + defense) on a selected concept and insight.
 * SSE stream with progress events + final result (critique, defense, improvedConcept).
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          console.error('[creative-debate] Error:', message);
    ...
    console.error('[POST /api/campaigns/wizard/strategy/creative-debate]', error);
