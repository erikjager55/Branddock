...
import { buildCreativePipelineContext, generateInsights } from '@/lib/campaigns/strategy-chain';
...
/**
 * POST /api/campaigns/wizard/strategy/mine-insights
 * Mines creative insights from brand context, personas, products, competitors and trends.
 * SSE stream with progress events + final insights result.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          console.error('[mine-insights] Error:', message);
    ...
    console.error('[POST /api/campaigns/wizard/strategy/mine-insights]', error);
