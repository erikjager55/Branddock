...
const secondaryNavItems = [
  ...
  },
  {
    ...
    href: "/strategy/content-calendar",
    icon: Calendar,
  },
  {
    label: "Goals & KPIs",
...
export default function StrategyLayout({
  ...
  const activeItem = secondaryNavItems.find(
    (item) =>
      pathname === item.href ||
      (item.href !== "/strategy" && pathname.startsWith(item.href))
  );

  // Don't show sidebar on the base /strategy page
  const isBasePage = pathname === "/strategy";
  if (isBasePage) {
  ...
  return (
    <div className="flex flex-col md:flex-row md:gap-8">
      ...
      <aside className="w-full md:w-52 flex-shrink-0">
        ...
        >
          <span>{activeItem?.label || "Strategy"}</span>
          <ChevronDown
            className={cn(
