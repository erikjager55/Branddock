...
export function CreditBalanceCard() {
  ...
  return (
    <Card padding="lg">
      ...
      {data.isLocked ? (
        <div className="mt-4 flex items-center gap-2 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-800">
          <Clock className="h-3.5 w-3.5 flex-shrink-0" />
          <span>
            {t('credits.trialLocked', {
              defaultValue:
                'Je trial is afgelopen — genereren en nieuwe items aanmaken staan op slot. Je merkdata blijft veilig en zichtbaar.',
            })}
          </span>
        </div>
      ) : (
        // isTrialing-guard: een geconverteerde org houdt trialEndsAt —
        // de trial-banner hoort dan weg (geen "nog X dagen"-ruis).
        data.isTrialing &&
