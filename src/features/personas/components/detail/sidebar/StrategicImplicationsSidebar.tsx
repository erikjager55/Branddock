    ...
    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className="flex items-center gap-2 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold text-gray-900">{t('strategic.title')}</h3>
          <p className="text-[10px] text-gray-500">{t('strategic.subtitle')}</p>
        </div>
      </div>
      ...
      {showConfirm && (
        <div className="mb-3 bg-amber-50 border border-amber-200 rounded-lg p-3">
          <p className="text-xs text-amber-800 mb-2">{t('strategic.regenerateShort')}</p>
      ...
      {isGenerating && (
        <div className="py-6 text-center">
          ...
          <p className="text-xs text-gray-500">{t('strategic.generatingShort')}</p>
        </div>
      )}
      ...
      {!isGenerating && implications && (
        <div className="space-y-2.5">
          ...
          {implications.length > 4 && (
            <p className="text-[10px] text-gray-400 text-center">{t('strategic.moreCount', { count: implications.length - 4 })}</p>
          )}
        </div>
      )}
      ...
      {!isGenerating && !hasContent && onGenerate && (
        <div className="py-4 text-center">
          ...
          <p className="text-xs text-gray-400 mb-2">{t('strategic.noImplicationsYet')}</p>
          <button
            onClick={onGenerate}
