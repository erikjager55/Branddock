  ...
  return (
    <div className="flex items-center gap-4 px-5 py-3.5">
      ...
      <div
        ...
        aria-label={t("systemRoles.row.swatchAlt", { role })}
      />
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {isOverride && (
            <span className="inline-flex items-center gap-1 text-[11px] font-medium text-teal-700 bg-teal-50 px-1.5 py-0.5 rounded">
              <Pencil className="w-3 h-3" /> {t("systemRoles.row.overrideBadge")}
            </span>
          )}
          {wcagWarning && (
            <span className="inline-flex items-center gap-1 text-[11px] font-medium text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded" title={wcagWarning.message}>
              ...
              {t("systemRoles.row.wcagBadge", { ratio: wcagWarning.contrastRatio?.toFixed(1) ?? '?' })}
            </span>
          )}
        </div>
        <p className="text-xs text-gray-500 mt-0.5">{description}</p>
        {source && (
          <p className="text-[11px] text-gray-400 mt-0.5 font-mono">{t("systemRoles.row.fromSource", { source })}</p>
        )}
      </div>
      ...
      {canEdit && (
        <Button variant="ghost" size="sm" onClick={onEdit}>
          ...
          {t("systemRoles.row.overrideAction")}
        </Button>
      )}
    </div>
  );
